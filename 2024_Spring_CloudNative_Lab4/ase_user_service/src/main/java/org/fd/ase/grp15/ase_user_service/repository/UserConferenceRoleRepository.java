...
public interface UserConferenceRoleRepository extends JpaRepository<UserConferenceRole, UserConferenceRole.UserConferenceRoleId> {
    ...
    @Query("select ucr from UserConferenceRole ucr where ucr.id.conferenceName = ?1 and ucr.id.role = 'PC_MEMBER'")
    public List<UserConferenceRole> findAllByConferenceNameAndRole(String conferenceName);
}
